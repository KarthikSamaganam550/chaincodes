...
public final class FabCar implements ContractInterface {
	...
	private enum FabCarErrors {
		CAR_NOT_FOUND, CAR_ALREADY_EXISTS, CAR_DOES_NOT_EXISTS
	}
	 ...
	 */
	@Transaction()
	public Car updateCar(final Context ctx, final String key, final String make, final String model, final String color,
			final String owner) {
		ChaincodeStub stub = ctx.getStub();
	...
	public Car[] queryCarsByColor(final Context ctx, final String color) {
		ChaincodeStub stub = ctx.getStub();
		/*
		 * final String startKey = "CAR0"; final String endKey = "CAR999";
		 */
		String query = "{\"selector\":{\"color\":\"" + color + "\"}}";
		List<Car> cars = new ArrayList<Car>();

		// QueryResultsIterator<KeyValue> results = stub.getStateByRange(startKey,
		// endKey);
	...
	/**
	 * Retrieves car history from the ledger
	 *
	 * @param ctx   the transaction context
	 * @param key   the car identifier
	 * @return array of Cars found on the ledger
	 */
	@Transaction()
	public CarHistory[] queryCarsHistroryById(final Context ctx, final String key) {
		ChaincodeStub stub = ctx.getStub();

		List<CarHistory> carHistories = new ArrayList<CarHistory>();

		QueryResultsIterator<KeyModification> carHistorys = stub.getHistoryForKey(key);
		for (KeyModification carState : carHistorys) {
			
			Car car = genson.deserialize(carState.getStringValue(), Car.class);
			CarHistory carHistory = new CarHistory(car, carState.getTimestamp(), carState.getTxId());

			carHistories.add(carHistory);
		}

		CarHistory[] response = carHistories.toArray(new CarHistory[carHistories.size()]);
